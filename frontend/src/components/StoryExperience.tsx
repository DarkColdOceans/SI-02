...
export function StoryExperience() {
  ...
  return <div className="motion-site">
    ...
    <main>
      <section className="motion-hero" id="top"><div className="motion-intro"><p className="micro-label">ONE MATERIAL / SIX CHAPTERS</p><h1>FROM<br /><em>POWDER</em><br />TO PRODUCT.</h1><p>A tiny material journey through mix, press, coat, check, and pack.</p><button className="motion-start" onClick={started ? replay : start}>{started ? 'REPLAY JOURNEY →' : 'START JOURNEY →'}</button></div><Continuous2DJourney state={twin.factoryState} activeId={activeId} started={started} onStage={selectChapter} /></section>
      ...
      <section className="motion-experiment"><div><span className="micro-label">OPTIONAL EXPERIMENT</span><h2>What if<br /><em>the pace changes?</em></h2><p className="scenario-intro">Combine changes, run the line forward, and compare the result with the untouched baseline.</p></div><ScenarioPanel liveState={twin.factoryState} liveHistory={twin.liveHistory} /></section>
    </main>
